...
import { getSpeakerById2025 } from "./speakers";

export const program2025: Record<string, Talk[]> = {
  "Day 1": [
    {
      time: "9:45",
...
export const getTalkSpeakers2025 = (talk: Talk): Speaker[] => {
  return talk.speakerIds.map((id) => getSpeakerById2025(id)).filter((speaker): speaker is Speaker => speaker !== undefined);
};

export const getSpeakerTalks2025 = (speakerId: string): Talk[] => {
  const allTalks: Talk[] = [];
  Object.values(program2025).forEach((dayTalks) => {
